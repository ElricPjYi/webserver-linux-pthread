...
int http_conn::m_epollfd=-1; //所有socket的事件都被注册到同一个epoll对象
...
http_conn::HTTP_CODE http_conn::process_read(){
    ...
    while(((m_check_state==CHECK_STATE_CONTENT)&&(line_status==LINE_OK))||((line_status=parse_line())== LINE_OK)){
        //主状态机处于检查请求体，从状态机检查到了完整的一行数据 或者调用解析之后，解析到了请求体 是OK就可以
        text=get_line();
    ...
    return NO_REQUEST;//如果switch没有执行
} //解析http请求
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
    m_url=strpbrk(text," \t");//判断空格和制表符哪个先出现，返回出现位置
    *m_url++ = '\0';
    char* method = text;
    ...
    *m_version++ = '\0';
    ...
    if(strncasecmp(m_url,"http://", 7)==0){
        ...
        m_url=strchr(m_url,'/');//找斜线第一次出现的索引 /index.html
    ...
    m_check_state=CHECK_STATE_HEADER;//检查状态变成请求头
